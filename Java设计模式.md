...
![](image/Java设计模式.assets/demo.png)
...
我们运行一下这段代码就会发现，假如我们把一个普通长方形作为参数传入resize方法，就会看到长方形宽度逐渐增长的效果，当宽度大于长度,代码就会停止，这种行为的结果符合我们的预期；假如我们再把一个正方形作为参数传入resize方法后，就会看到**正方形的宽度和长度都在不断增长，代码会一直运行下去**，直至系统产生溢出错误。所以，普通的长方形是适合这段代码的，正方形不适合。
...
<img src="image/Java设计模式.assets/依赖倒转原则.png" style="zoom:80%;" />
...
> 注意：在电脑类中，写死了各个硬件的品牌，合理的写法应该是为各个硬件写一个接口
...
<img src="image/Java设计模式.assets/image-20231103203405804.png" alt="image-20231103203405804" style="zoom: 50%;" />
...
<img src="image/Java设计模式.assets/合成复用原则1.png" alt="image-20191229173554296" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/image-20200401222951963.png" style="zoom:67%;" />
...
<img src="image/Java设计模式.assets/抽象工厂模式.png" style="zoom:67%;" />
...
如果要加**同一个产品族的话，只需要再加一个对应的工厂类即可**，不需要修改其他的类。
...
<img src="image/Java设计模式.assets/建造者模式1.png" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/适配器模式.png" style="zoom:80%;" />

> 适配器 SDAdapterTF  目标：SDCard  适配者：TFCard
>
> 适配器类 **继承**适配者类 并 实现目标接口
...
缺点：类适配器模式**违背了合成复用原则**。只有在目标类**有一个接口**规范的情况下**可用**，反之不可用。

> 即若无SDCard接口，只有SDCardImpl，SDAdapterTF是无法继承两个类的

改进：对象适配器模式（将被适配者**聚合**进适配器，而不是采用**继承**，即**遵循合成复用原则**）
...
<img src="image/Java设计模式.assets/对象适配器模式.png" style="zoom:80%;" />

> 适配器类 **聚合/组合**适配者类 并 实现目标接口(或继承目标类)
...
### 5.2.6 JDK源码解析-InputStreamReader
...
![](image/Java设计模式.assets/适配器模式-jdk源码解析.png)
...
<img src="image/Java设计模式.assets/装饰者模式-使用前.png" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/装饰者模式.png" style="zoom:75%;" />

> 重点：抽象装饰类(Garnish) **继承**和**聚合/组合** 抽象构件类(FastFood)，而不是采用逐层继承的方式
...
<img src="image/Java设计模式.assets/装饰者模式-jdk源码.png" style="zoom:80%;" />
...
* 不同点：
  * 目的不同
    装饰者是为了**增强目标对象**
    静态代理是为了**保护和隐藏目标对象**，**增强目标方法**
  * 获取目标对象构建的地方不同
    装饰者是由**外界传递进来**，可以通过构造方法传递
    静态代理是在**代理类内部创建**，以此来隐藏目标对象

> 我的理解：
>
> 1. 抽象工厂模式，是生产一个产品族的东西，如汽车的各个零件
> 2. 建造者模式，是按照一定的步骤，一步一步的组装成一个复杂对象，如将汽车的各个零件组装成一辆汽车
> 3. 代理模式，是内部自己创建目标对象，用于保护和隐藏目标对象，增强的是方法
> 4. 装饰者模式，是按照自己的需求，装饰一个对象，且装饰者是由外界传进来的，增强的是对象
...
![](image/Java设计模式.assets/image-20200207194617620.png)
...
<img src="image/Java设计模式.assets/桥接模式.png" style="zoom:80%;" />
...
* 当一个系统需要在构件的抽象化角色和具体化角色之间增加更多的灵活性时。避免在两个层次之间建立静态的继承联系，通过桥接模式可以使它们在抽象层建立一个关联关系（聚合、组合关系）。
...
![](image/Java设计模式.assets/外观模式引入.jpg)
...
<img src="image/Java设计模式.assets/外观模式.png" style="zoom:80%;" />
...
### 5.5.5 Tomcat源码解析-ServletRequest

使用tomcat作为web容器时，接收浏览器发送过来的请求，tomcat会将请求信息封装成ServletRequest对象，如下图①处对象。但是大家想想ServletRequest是一个接口，它还有一个子接口HttpServletRequest，而我们知道该request对象肯定是一个HttpServletRequest对象的子实现类对象，到底是哪个类的对象呢？可以通过输出request对象，我们就会发现是一个名为RequestFacade的类的对象。

<img src="image/Java设计模式.assets/image-20200207234545691.png" style="zoom:60%;" />

RequestFacade类就使用了外观模式。先看结构图：

<img src="image/Java设计模式.assets/外观模式-jdk源码解析.png" style="zoom:70%;" />
...
<img src="image/Java设计模式.assets/image-20200208180417291.png" style="zoom:60%;" />
...
<img src="image/Java设计模式.assets/image-20200208182322313.png" style="zoom:80%;" />

要实现该案例，我们先画出类图：

<img src="image/Java设计模式.assets/组合模式.png" style="zoom:80%;" />
...
* 安全组合模式
  ...
  <img src="image/Java设计模式.assets/组合模式-安全性.png" style="zoom:80%;" />
...
> 享元角色具有两种状态：内部状态 和 外部状态（会随着环境发生改变）
...
<img src="image/Java设计模式.assets/俄罗斯方块.jpeg" style="zoom:60%;" />



**先来看类图：**

<img src="image/Java设计模式.assets/享元模式.png" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/image-20200208212930857.png" style="zoom:80%;" />
...
行为型模式用于**描述程序在运行时复杂的流程控制**，即描述多个类或对象之间怎样相互协作共同完成单个对象都无法单独完成的任务，它涉及算法与对象间职责的分配。
...
**是唯二之一的类行为型模式**。在面向对象程序设计过程中，程序员常常会遇到这种情况：设计一个系统时知道了算法所需的关键步骤，而且确定了这些步骤的执行顺序，但某些步骤的具体实现还未知，或者说某些步骤的实现与具体的环境相关。
...
> 模板方法模式是对父类的继承，算法骨架已经确定，而且该骨架已经含有一些步骤的具体实现，子类只需实现父类的抽象方法即可
>
> 适用于流程固定，但是其中的**一些步骤需要根据具体情况进行替换或者扩展**的场景。
...
<img src="image/Java设计模式.assets/模板方法模式.png" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/image-20200210143039168.png" style="zoom:80%;" />

作为一个程序猿，开发需要选择一款开发工具，当然可以进行代码开发的工具有很多，可以选择Idea进行开发，也可以使用eclipse进行开发，也可以使用其他的一些开发工具。

<img src="image/Java设计模式.assets/image-20200210144457478.png" style="zoom:70%;" />
...
<img src="image/Java设计模式.assets/策略模式.png" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/image-20200211130313251.png" style="zoom:60%;" />
...
<img src="image/Java设计模式.assets/命令模式.png" style="zoom:75%;" />
...
* 抽象处理者（Handler）角色：定义一个处理请求的接口或抽象类，包含抽象处理方法和一个后继连接。
...
<img src="image/Java设计模式.assets/责任链模式.png" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/状态模式前.png" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/状态模式.png" style="zoom:70%;" />
...
<img src="image/Java设计模式.assets/观察者模式.png" style="zoom:80%;" />
...
### 6.6.6 JDK中提供的实现-Observable和Observer
...
<img src="image/Java设计模式.assets/image-20200214110924010.png" style="zoom:60%;" />
...
<img src="image/Java设计模式.assets/中介者模式.png" style="zoom:70%;" />
...
<img src="image/Java设计模式.assets/迭代器模式.png" style="zoom:90%;" />
...
![](image/Java设计模式.assets/访问者模式.png)
...
<img src="image/Java设计模式.assets/image-20200214215446638.png" style="zoom:70%;" />
...
<img src="image/Java设计模式.assets/image-20200426233931693.png" style="zoom:67%;" />
...
<img src="image/Java设计模式.assets/白箱备忘录模式.png" style="zoom:80%;" />
...
<img src="image/Java设计模式.assets/黑箱备忘录模式.png" style="zoom:70%;" />
...
<img src="image/Java设计模式.assets/image-20200215220322641.png" style="zoom:60%;" />
...
<img src="image/Java设计模式.assets/image-20200215225227616.png" style="zoom:50%;" />
...
<img src="image/Java设计模式.assets/解释器模式.png" style="zoom:80%;" />
...
* 编写配置文件。在类路径下编写一个名为ApplicationContext.xml的配置文件
  ...
  <img src="image/Java设计模式.assets/image-20200429165544151.png" style="zoom:60%;" />
...
<img src="image/Java设计模式.assets/image-20200429111324770.png" style="zoom:40%;" />
...
* spring-context模块构架于核心模块之上，扩展了BeanFactory，为它添加了Bean生命周期控制、框架事件体系及资源加载透明化等功能。此外，该模块还提供了许多企业级支持，如邮件访问、远程访问、任务调度等，**ApplicationContext 是该模块的核心接口，它的超类是 BeanFactory**。与BeanFactory不同，**ApplicationContext实例化后会自动对所有的单实例Bean进行实例化与依赖关系的装配(与BeanFactory对立)**，使之处于待用状态。
  ...
  /*
  	使用ApplicationContext，在创建applicationContext时就会创建UserService和UserDao
  	而使用BeanFactory，在创建beanFactory后，获取对应Bean时才会创建对应对象
  */
  ```
...
<img src="image/Java设计模式.assets/image-20200430220155371.png" style="zoom:60%;" />

要知道工厂是如何产生对象的，我们需要看具体的IoC容器实现，Spring提供了许多IoC容器实现，比如：

* ClasspathXmlApplicationContext : 根据**类路径加载xml**配置文件，并创建IOC容器对象。
* FileSystemXmlApplicationContext :根据**系统路径加载xml**配置文件，并创建IOC容器对象。
* AnnotationConfigApplicationContext :加载**注解类配置**，并创建IOC容器。
...
> 即配置文件(如xml文件)中定义的Bean会被封装为BeanDefinition对象（由后面的BeanDifinitionReader进解析xml文件然后注册到注册表BeanDefinitionRegistry中）
...
<img src="image/Java设计模式.assets/image-20200429204239868.png" style="zoom:60%;" />
...
<img src="image/Java设计模式.assets/image-20200429204700956.png" style="zoom:60%;" />

> BeanDefinitionReader用来解析xml文件，并将xml文件中定义的bean封装成BeanDefinition对象，然后注册到BeanDefinitionRegistry中
...
B**eanDefinitionReader用来解析bean定义，并封装成BeanDefinition对象**，而我们定义的配置文件中定义了很多bean标签，所以就有一个问题，解析的**BeanDefinition**对象**存储到**哪儿？答案就是BeanDefinition的注册中心，而该注册中心顶层接口就是**BeanDefinitionRegistry**。

> 由BeanDefinitionReader解析xml文件，封装成的BeanDefinition便是注册到该接口的实现类中
...
<img src="image/Java设计模式.assets/image-20200429211132185.png" style="zoom:50%;" />![image-20231108224100562](image/Java设计模式.assets/image-20231108224100562.png)
...
* DefaultListableBeanFactory
  ...
  DefaultListableBeanFactory既实现了BeanFactory接口，又实现了BeanDefinitionRegistry接口，所以**既是容器，又是注册表**
...
* GenericApplicationContext
  ...
  GenericApplicationContext既实现了ApplicationContext(实现了BeanFactory接口)接口，又实现了BeanDefinitionRegistry接口，所以**既是容器，又是注册表**

  > 这个是我自己加到笔记里的，但是这个类源码里没找到声明的beanDefinitionMap
...
用于封装bean的属性，体现到上面的配置文件就是**封装xml文件中bean标签的子标签property标签数据。**
...
**一个bean标签可以有多个property子标签**，所以再定义一个MutablePropertyValues类，用来存储并**管理多个PropertyValue对象。**
...
BeanDefinition类用来**封装xml文件中的bean标签信息的**，主要包含id（即bean对象的名称）、class（需要交由spring管理的类的全类名）及子标签property数据。
...
public abstract class AbstractApplicationContext implements ApplicationContext {
    ...
    protected static Map<String, Object> singletonObjects = new HashMap<String, Object>();
...
在模块spring_demo中，在pom.xml文件中引入依赖：
